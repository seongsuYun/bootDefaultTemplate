...
scheduler.xy = {min_event_height: 40,scale_width: 61,scroll_width: 18,scale_height: 30,month_scale_height: 20,menu_width: 25,margin_top: 0,margin_left: 0,editor_width: 140};
//Jin.Lee scale_width:50 -> 61

scheduler.keys = {edit_save: 13,edit_cancel: 27};
scheduler.set_sizes = function() {
	var a = this._x = this._obj.clientWidth +1 - this.xy.margin_left, b = this._y = this._obj.clientHeight - this.xy.margin_top, c = this._table_view ? 0 : this.xy.scale_width + this.xy.scroll_width, d = this._table_view ? -1 : this.xy.scale_width;
	...
	week_start: function(a) {
		...
		a = a.replace(/%[a-zA-Z]/g, function(a) {
			switch (a) {
				...
				case "%y":
					return '"+scheduler.date.to_fixed(date.getFullYear()%80)+"';
				case "%Y":
				...
				case "%a":
					return '"+(date.getHours()>11?"' + Alfresco.util.message('swp.com.label.pm') + '":"' + Alfresco.util.message('swp.com.label.am') + '")+"';
				case "%A":
					return '"+(date.getHours()>11?"' + Alfresco.util.message('swp.com.label.pm') + '":"' + Alfresco.util.message('swp.com.label.am') + '")+"';
				case "%s":
...
scheduler.locale = {date: {month_full: "01,02,03,04,05,06,07,08,09,10,11,12".split(","),month_short: "01,02,03,04,05,06,07,08,09,10,11,12".split(","),day_full: "일,월,화,수,목,금,토".split(","),day_short: "일,월,화,수,목,금,토".split(",")},labels: {dhx_cal_today_button: "오늘",day_tab: "일간",week_tab: "주간",month_tab: "월간",new_event: "일정 제목",icon_save: "등록",icon_cancel: "취소",icon_details: "Details",
		...
		year_tab: "1년보기",week_agenda_tab: "Agenda",grid_tab: "Grid",drag_to_create: "Drag to create",drag_to_move: "Drag to move"}};
scheduler.config = {default_date: "%Y.%M.%j",month_date: "%Y.%F ",load_date: "%Y-%m-%d",week_date: "%l",day_date: "%j(%D)",hour_date: "%H:%i",month_day: "%j",xml_date: "%m/%d/%Y %H:%i",api_date: "%d-%m-%Y %H:%i",preserve_length: !0,time_step: 5,start_on_monday: 1,first_hour: 0,last_hour: 24,readonly: !1,drag_resize: 1,drag_move: 1,drag_create: 1,dblclick_create: 1,edit_on_create: 1,details_on_create: 0,cascade_event_display: !1,cascade_event_count: 4,cascade_event_margin: 30,multi_day: !0,multi_day_height_limit: 0,drag_lightbox: !0,
...
scheduler.init_templates = function() {
	...
	d(scheduler.templates, {day_date: b(c.default_date),month_date: b(c.month_date),week_date: function(a, b) {
		...
		month_date_class: function() {
			...
			//Jin.Lee 추가
			var c = scheduler.config.max_month_events;
			var e = b - c;
			return "<a  href='#?w=271' rel='layerMore' class='poplink typeB dpth4 txtG6'>외" + e + "건</a>"
			//return "<a>" + e + "건</a>"
		}});
...
scheduler._render_month_link = function(a) {
	for (var b = this._rendered_location, c = this._lame_clone(a), d = a._sday; d < a._eday; d++) {
		...
		i.onclick = function(a) {
			//Jin.Lee 추가 : 외3건 클릭시 일간으로 이동을 막음.
...
scheduler._render_v_bar = function(a, b, c, d, e, f, g, h, k) {
	...
	if (k || !scheduler.renderEvent(q, j)) {
		...
		r += '<div class="dhx_body" style=" width:' + (d - (this._quirks ? 4 : 14)) + "px; height:" + (e - (this._quirks ? 20 : 30) + 1 -12 ) + "px;" + m + "" + o + '">' + h + "</div>";
		//Jin.Lee day,week event header 높이값 수정
...
scheduler.form_blocks = {template: {render: function(a) {
		...
		focus: function(a) {
			...
			if (f.full_day) {
				if (!a._full_day) {
					var k = Alfresco.util.message('swp.com.label.calendar');
					k +="<label class='dhx_fullday'><input type='checkbox' name='full_day' value='true'> " + scheduler.locale.labels.full_day + "&nbsp;</label></input>";
...
scheduler.getLightbox = function() {
	if (!this._lightbox) {
		...
		b += "<div class='dhx_btn_set_wrap'>";
		b += "<a href='#none' class='close'><img src='{0}res/images/swp/common/btn_ly_close1.gif' class='btn_close' alt='간략등록 레이어 닫기' /></a>".format(Alfresco.constants.URL_CONTEXT);

		for (b, c = this.config.buttons_left, d = 0; d < c.length; d++)
